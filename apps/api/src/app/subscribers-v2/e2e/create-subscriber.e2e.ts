...
describe('Create Subscriber - /subscribers (POST) #novu-v2', () => {
  ...
  it('should upsert an existing subscriber if the subscriberId matches', async () => {
    ...
    const { result: subscriber } = await novuClient.subscribers.create(payload1, payload1.subscriberId);
    ...
    const { result: updatedSubscriber } = await novuClient.subscribers.create(payload2, payload2.subscriberId);
    ...
    } = await novuClient.subscribers.search({});
